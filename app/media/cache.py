...
from app.core.config import CACHE_DIR, CACHE_PRUNE_TARGET_RATIO, MAX_CACHE_AGE_HOURS, MAX_CACHE_BYTES
...
def collect_cache_entries(active_track_ids: set[str]) -> list[dict[str, object]]:
...
def prune_cache(
    ...
    for entry in collect_cache_entries(active_track_ids):
    ...
    target_cache_bytes = min(max_cache_bytes, max(0, int(max_cache_bytes * CACHE_PRUNE_TARGET_RATIO)))
    entries = sorted(
        collect_cache_entries(active_track_ids),
